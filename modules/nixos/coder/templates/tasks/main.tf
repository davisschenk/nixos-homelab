# Coder Tasks template: same envbuilder/devcontainer build as ../docker,
# plus a Claude Code agent wired to Coder's Tasks feature so a workspace can
# be launched from a prompt instead of a manual shell session.
#
# claude-code module pinned to 4.9.2 deliberately, not `~>` or `latest` — v5
# dropped Tasks support (ai_prompt/task_app_id), see coder/registry PR #861.
#
# Not deployed via Nix — push with:
#   coder login https://coder.schenkenberger.dev
#   cd modules/nixos/coder/templates/tasks && terraform init
#   coder templates push claude-tasks --directory . --yes
...
locals {
  ...
  # the docker-dev template was: /var/lib/docker is wiped every boot.
...
# Tasks auto-applies --dangerously-skip-permissions (module's own documented
# behavior) — no interactive approval prompts, so the prompt text is the only
# gate on what the agent does in the workspace.
module "claude-code" {
...
# code-server so a finished task's work can still be inspected/continued
# manually, same as a docker-dev workspace.
module "code-server" {
  count     = data.coder_workspace.me.start_count
